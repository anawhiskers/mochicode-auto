...
| Bounded Luna Medium worker | Sizable independent implementation leaf whose parallelism, isolation, or batch volume can repay delegation overhead. | A real Luna Medium child implements; parent verifies; Max is escalation. |
| Sol-led fan-out | Frozen interfaces and at least two disjoint independent leaves. | Start with two, cap normal waves at three, integrate under Sol. |
